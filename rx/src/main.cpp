...
void decodePacket(uint8_t* packet, int packetSize) {
  if (packetSize < 32) {
    Serial.printf("[RX] WARNING: Short packet (%d bytes)\n", packetSize);
    return;
  }
...
void onReceive(int packetSize) {
  ...
  // Ensure we stay in continuous RX mode
  LoRa.receive();
}
...
void loop() {
  ...
  if (packetsReceived > 0 && (millis() - lastPacketTime > 5000)) {
    Serial.println("[RX] WARNING: No packets received for 5 seconds");
    delay(5000);
  }

  // Main work done in callback
  delay(10);
}
